...
const health = { status: 'ok', version: '0.3.6', loaded_models: [] };
...
test('request preserves Unicode/JSON, auth, routing options and proxy prefix', async () => {
  let calls = 0;
  const client = new Laya({ baseURL: 'https://example.com/laya///', apiKey: 'key', headers: { 'X-App': 'test' },
    fetch: async (url, init) => {
      calls++;
      assert.equal(url, 'https://example.com/laya/v1/predict');
      assert.equal(init.method, 'POST');
      assert.equal(init.headers.get('Authorization'), 'Bearer key');
      assert.equal(init.headers.get('Content-Type'), 'application/json');
      assert.equal(init.headers.get('X-App'), 'test');
      assert.deepEqual(JSON.parse(init.body), { state: { text: 'नमस्ते', count: 0 }, questions, model: 'en', task: 'typed', lang: 'hi' });
      return json(prediction);
    } });
  await client.predict({ text: 'नमस्ते', count: 0 }, questions, { model: 'en', task: 'typed', lang: 'hi' });
  assert.equal(calls, 1);
});

test('route accepts no questions and health uses GET', async () => {
  const calls = [];
  const client = new Laya({ fetch: async (url, init) => {
    calls.push([new URL(url).pathname, init.method, init.body]);
    return json(url.endsWith('/health') ? health : route);
  } });
  await client.route('Hello');
  assert.deepEqual(await client.health(), health);
  assert.deepEqual(calls, [['/v1/route', 'POST', '{"state":"Hello","questions":{}}'], ['/health', 'GET', undefined]]);
});
