...
export function sequenceWithState(prefix: QuestionPrefix, stateIds: number[], sepId: number,
  ...
  const kept = truncateLeft ? stateIds.slice(-room) : stateIds.slice(0, room);
  const ids = [...prefix.ids, ...kept, sepId].slice(0, maxLen);
  // Count against the final clamp rather than `kept`: with room == 0 a left-truncating
  // slice(-0) is the whole state, and ids.slice(0, maxLen) is what actually discards it.
