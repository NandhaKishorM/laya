...
export function validateHealth(value: unknown): void {
  expect(isRecord(value) && value.status === 'ok' && typeof value.version === 'string', 'health');
  expect(Array.isArray(value.loaded_models) && value.loaded_models.every(model), 'loaded_models');
}
...
export function validatePrediction(value: unknown, questions: Questions): void {
  ...
  validateRoute(value.routing);
  for (const [id, question] of Object.entries(questions)) {
    const answer = value.answers[id];
    expect(isRecord(answer) && answer.type === question.type, `answers.${id}.type`);
    expect(probability(answer.confidence) && isRecord(answer.action) && probability(answer.action.act_probability), `answers.${id}.confidence/action`);
