...
* **`confidence`** on `choice` and `score` answers is 1 minus normalised entropy, a measure of how concentrated the distribution is, not Jev's `(n·p_max − 1)/(n − 1)`. A threshold carried over from Jev does not transfer. Its value can also shift with the number of options, so validate any cutoff at the counts your workload uses. For one calibrated number on every question type, gate on `answer_confidence`, the probability of the reported answer.
