    ...
    for i, it in enumerate(items):
        ...
        if has_target and "target" in it:
            if len(it["target"]) > k:
                ...
                # actual mistake: a target with more entries than the item has options. Checked
                # against this item's own marker count k, not the batch-wide kmax, so a target
                # that overflows its own options can't hide under a wider sibling row.
                raise ValueError(
