...
test('JavaScript → HTTP → Python Router → real offline Agent inference', { timeout: 90_000 }, async t => {
  ...
  assert.equal((await client.route({ text: 'मुझे पैसे वापस चाहिए' })).model, 'multilingual');
  assert.equal((await client.predict({ text: 'मुझे पैसे वापस चाहिए' }, fixture.questions)).routing.model, 'multilingual');
  assert.equal((await client.route('hello', {}, { task: 'typed_decisions' })).model, 'typed-decisions');
  ...
  await assert.rejects(new Laya({ baseURL: fixture.baseURL }).health(), error => error instanceof LayaAPIError && error.status === 401);
  await assert.rejects(client.route('hello', {}, { model: 'unknown' }), error => error instanceof LayaAPIError && error.status === 422);
});
