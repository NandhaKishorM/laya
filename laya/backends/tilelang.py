"""The TileLang backend: fused kernels + bf16 resident weights + one CUDA graph per shape bucket.

    agent = laya.load("convaiinnovations/laya", backend="tilelang")   # fast=True, or agent.accelerate()

Requires CUDA, `pip install laya[fast]` (tilelang) and a ModernBERT-family encoder. `FastLaya` is the
forward itself (it used to live in `laya/fast.py`, which still re-exports it); `TileLangBackend` is
the contract wrapper the Agent drives.

The kernels are bf16 (`laya/tl_kernels.py`), so this backend runs only when `agent.dtype` is bf16;
an fp16 agent gets the eager forward and one warning (fp16 kernels are proposed in #467).
"""
import sys
import threading
import torch
...
from .base import DYNAMIC_MAX_L, LONG_BUCKET, Backend, BackendUnavailable, bucket_rows, pad_batch
...
class FastLaya:
    ...
    # ------------------------------------------------------------------ kernels per shape
    DYNAMIC_MAX_L = DYNAMIC_MAX_L   # up to here one dynamic-shape attention kernel is as fast as a static one
    LONG_BUCKET = LONG_BUCKET       # beyond it, static kernels per (B, L) bucket of this size
    ...
    def _forward_unlocked(self, input_ids, attention_mask, marker_pos, marker_mask, qtype, detach_encoder=False):
        m = self.m
        # the shared padding policy (laya.backends.base): rows to a power of two, tokens to a
        # 16- or 64-token bucket, capped at max_len; padding rows are token 0 with length 0
        ids, att, _, _, qt, N, L0 = pad_batch(input_ids, attention_mask, marker_pos, marker_mask, qtype,
                                              self.max_len, pad_id=0)
        lens = att.sum(1).to(torch.int32)
...
class TileLangBackend(Backend):
    ...
    def _install(self):
        ...
        if agent.dtype != BF:
            raise BackendUnavailable("the TileLang kernels are bf16 and the agent runs %s" % (agent.dtype,))
        ...
        for _attempt in range(2):  # tilelang's JIT cache has been seen to fail once, then succeed
            try:
                self.fast = FastLaya(self.model, max_len=agent.cfg.get("max_len", 512),
                                     use_graphs=self.use_graphs, verbose=self.verbose)
                break
    ...
    def warmup(self, shapes=None):
        ...
        for rows, tokens in shapes or ((1, 64), (4, 128)):
            ids = torch.full((rows, tokens), 1, dtype=torch.long, device=self.agent.device)
            att = torch.ones((rows, tokens), dtype=torch.long, device=self.agent.device)
            mpos = torch.zeros((rows, 2), dtype=torch.long, device=self.agent.device)
            mmask = torch.ones((rows, 2), dtype=torch.bool, device=self.agent.device)
            qt = torch.zeros((rows,), dtype=torch.long, device=self.agent.device)
